...
export interface RegisterRequest {
  name: string;
  email: string;
  /** Opcional. Se omitido, uma senha provisória é gerada e o usuário recebe e-mail para definir a senha. */
  password?: string;
  role: RegisterRequestRole;
  cpf: string;
  phone: string;
  /** Token reCAPTCHA v2 (resposta do widget). */
...
export interface Lesson {
  ...
  /**
   * S3 object key (set after presigned upload); mutually exclusive with videoUrl for hosted video
   * @nullable
   */
  videoObjectKey?: string | null;
  /**
   * Presigned GET URL for preview (teacher) or playback when applicable
   * @nullable
   */
...
/**
 * Headers the client must send on PUT (e.g. Content-Type)
 */
export type PresignVideoUploadResponseHeaders = { [key: string]: string };

export interface PresignVideoUploadResponse {
  /** Presigned URL for HTTP PUT of the raw video bytes */
  uploadUrl: string;
  /** S3 object key to save on the lesson as videoObjectKey */
  objectKey: string;
  /** Headers the client must send on PUT (e.g. Content-Type) */
  headers: PresignVideoUploadResponseHeaders;
}

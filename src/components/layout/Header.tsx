...
export function Header({ user, theme, onMenuToggle, onToggleTheme, onLogout }: HeaderProps) {
  ...
  return (
    ...
    >
      ...
      <div className="relative flex h-full items-center justify-end gap-1 pr-3 sm:gap-2 sm:pr-4 md:hidden">
        <button
          type="button"
        ...
        <div
          className={cn(
            ...
            "pl-[3.25rem] pr-[8.25rem] sm:pl-14 sm:pr-[9.5rem]",
          )}
        >
          <img
        ...
        <HeaderActions {...actionsProps} className="relative z-20" />
      </div>

      {/* ── Desktop/tablet (md+): layout original — imagem à esquerda ── */}
      <div className="hidden h-full items-center justify-between gap-3 pl-1 pr-4 sm:pl-1.5 sm:pr-6 md:flex">
